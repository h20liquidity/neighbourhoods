...
// String version of factory address.
    ...
    "usdt-token-address: 0xc2132D05D31c914a87C6611C10748AEb04B58e8F,"
    // String version of approved counterparty.
    "approved-counterparty: 0x1F8Cd7FB14b6930665EaaA5F5C71b9e7396df036," "actual-counterparty: context<1 2>(),"
    // Check that
    // - counterparty is approved.
    // - nht token address is the token in to ob.
    // - usdt token address is the token out from ob.
    ":ensure(" " equal-to(approved-counterparty actual-counterparty)" " equal-to(context<3 0>() nht-token-address)"
    " equal-to(context<4 0>() usdt-token-address)" "),"
    // Order hash.
    "order-hash: context<1 0>(),"
    // Figure out when the order started.
    "order-init-time-key: hash(order-hash 0)," "order-init-time: any(get(order-init-time-key) block-timestamp()),"
    ":set(order-init-time-key order-init-time),"
    // We buy $50 worth of nht for usdt per hour.
    // 50e6 is $50 in usdt.
    // 50e6 / 3600 is $50 per hour.
    "usdt-per-second: 13889," "total-time: int-sub(block-timestamp() order-init-time),"
    "max-usdt-amount: int-mul(total-time usdt-per-second)," "current-usdt-amount-key: hash(order-hash 1),"
    "current-usdt-amount: get(current-usdt-amount-key),"
    "target-usdt-amount: int-sub(max-usdt-amount current-usdt-amount),"
